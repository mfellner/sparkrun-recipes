...
def metric(base):
 m=RX.search(get(base.rstrip('/')+'/metrics')); return float(m.group(1)) if m else 0.0
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--direct-url',default='http://127.0.0.1:8000');ap.add_argument('--proxy-url',default='http://192.168.1.95:4000');ap.add_argument('--recipe',required=True);ap.add_argument('--video',required=True);ap.add_argument('--out',required=True);a=ap.parse_args()
 tag=hashlib.sha256(str(time.time_ns()).encode()).hexdigest()[:12]; rec={'schema':1,'cluster_id':'sparkrun_d56463b6b176dd58_009b8bd44277','recipe_sha256':hashlib.sha256(Path(a.recipe).read_bytes()).hexdigest(),'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'started_at':time.time(),'checks':{}}
 u=f'FINAL-APC-{tag} '+('alpha '*7600)+' Reply exactly FINAL_APC_BASE.'; cold=post(a.direct_url,body([{'role':'user','content':u}])); pre=metric(a.direct_url); follow=post(a.direct_url,body([{'role':'user','content':u},{'role':'assistant','content':'FINAL_APC_BASE'},{'role':'user','content':'Reply exactly FINAL_APC_FOLLOW.'}])); postm=metric(a.direct_url);rec['checks']['apc']={'passed':content(cold)=='FINAL_APC_BASE' and content(follow)=='FINAL_APC_FOLLOW' and postm-pre>0,'hit_delta':postm-pre,'cold':cold,'follow':follow}
