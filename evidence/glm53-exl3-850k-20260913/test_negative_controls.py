...
def sanitizer_negative_controls() -> None:
    ...
    scheme_relative = sanitize(
        {"stdout": f"connect //synthetic-user:{marker}@example.invalid/path"}
    )
    sshpass_argv = sanitize(
        {"argv": ["sshpass", "-p", marker, "ssh", "example.invalid"]}
    )
    sshpass_attached = sanitize(
        {"argv": ["/usr/bin/sshpass", f"-p{marker}", "ssh", "example.invalid"]}
    )
    dangling = sanitize({"argv": ["tool", "--api-key"]})
    lone_surrogate = sanitize({"value": "\ud800"})
    for payload in (scheme_relative, sshpass_argv, sshpass_attached, dangling, lone_surrogate):
        if marker in json.dumps(payload, ensure_ascii=True) or sanitize(payload) != payload:
            raise AssertionError("credential edge-case sanitizer did not fail closed")
    print("rejected: scheme-relative userinfo, sshpass passwords, dangling options, and invalid Unicode")

    mapping_aliases = (
        {"auth": marker},
        {"basicAuth": marker},
        {"ssh_key": marker},
        {"api_key": marker, "payload": marker},
        {"api_key": marker, "body": marker},
    )
    for candidate in mapping_aliases:
        sanitized = sanitize(candidate)
        if marker in json.dumps(sanitized, ensure_ascii=True):
            raise AssertionError("credential mapping alias or sibling payload survived")
    print("rejected: auth aliases and sensitive-mapping payload/body siblings")
...
def main() -> int:
    ...
    rejected(
        "readiness-to-acceptance allocation failure with refreshed manifest",
        lambda root: mutate_json(
            root / "runtime.json",
            lambda data: data["hosts"]["192.168.178.47"]
            ["kernel_readiness_to_acceptance"].__setitem__(
                "stdout", "NVRM: Out of memory [NV_ERR_NO_MEMORY]\n"
            ),
        ),
        "192.168.178.47 readiness-to-acceptance kernel safety",
    )
    for kernel_name in (
        "kernel_since_launch",
        "kernel_readiness_to_acceptance",
        ...
        rejected(
            f"{kernel_name} warning on stderr with refreshed manifest",
            lambda root, name=kernel_name: mutate_json(
                root / "runtime.json",
                lambda data: data["hosts"]["192.168.178.47"][name].__setitem__(
                    "stderr", "journalctl: journal file corrupted, ignoring file\n"
                ),
            ),
            f"192.168.178.47 {kernel_name} stderr",
        )
    rejected(
