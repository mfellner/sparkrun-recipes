...
- SparkRun workload: `sparkrun_f906ee990596486e_20260913c411`
- Acceptance run ID: `f906c41120260913`
- Acceptance interval: `2026-09-13T17:17:34.233669+02:00` through `2026-09-13T17:18:12.275194+02:00`
- Runtime capture completed: `2026-09-13T18:14:14.895732+02:00`
- Mia source: `f906ee990596486e10ddbe381efa6f0e496f77e3`
...
- Upstream compatibility: 114 pytest checks and 13 subtests passed; the standalone APC composition gate passed against the exact pristine base-image runtime
- Runtime overlay: latest hybrid DFlash replay clamp and per-group APC retention patches applied and verified on both ranks; optional per-group retention remains unset/off

## What the live receipts prove

`launch-receipt.json` was recorded against the exact launch epoch and binds the
current recipe SHA-256, exact reviewed YAML command, external mod-manifest
digest, deterministic container name, and launch argv. `runtime.json` then
binds SparkRun's raw recipe command and both rank-specific wrapper/runtime argv
back to that receipt.
...
and the selected fatal-signature list is empty. The runtime patch verifier also
proves the exact composed hybrid APC, DFlash replay, per-group retention,
block-priority, and prior-boundary source state on both ranks. Full serve logs
may be redacted rather than committed when they contain unsafe command-like
content.
...
The fail-closed readiness marker was written only after the gate completed both
the C4/32-token acceptance shape and the deterministic 110,035-token retrieval
shape. The per-rank kernel receipts are clean for `NV_ERR_NO_MEMORY`, NVIDIA
Xid, and kernel OOM-kill signatures throughout the complete readiness-to-
acceptance interval; bounded allocation probes before readiness are reported
separately by `runtime.json`.
