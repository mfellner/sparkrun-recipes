...
SAFE_PROXY_COMMAND_SHA256 = "99eedd47d2adbc5582743277d2ac2fbdbefcc187dc46ea44aaa1fce8e61a0422"
...
def redact_text(
    ...
    *,
    ...
    try:
        ...
        return REDACTED
    if encoded_size > _json_max_size:
        return REDACTED
    sanitized = text
...
def sanitize_argv(
    ...
    *,
    ...
    first_scalar, first_converged = (
        decode_serialized_argv_fragment(argv[0]) if argv else (None, True)
    )
    sshpass_command = (
        first_converged
        and isinstance(first_scalar, str)
        and Path(first_scalar).name == "sshpass"
    )
    for item in argv:
    ...
    for item in argv:
        ...
        if not isinstance(decoded_item, str):
            result.append(REDACTED)
            continue
        if sshpass_command and decoded_item in {"-p", "--password"}:
            result.append(item)
            redact_next = True
            continue
        if sshpass_command and (
            decoded_item.startswith("--password=")
            or re.fullmatch(r"-p.+", decoded_item, re.DOTALL)
        ):
            result.append(REDACTED)
            continue
...
def sanitize_evidence(
    ...
    *,
    ...
    if isinstance(value, dict):
        ...
        sibling_payload_roles = {
            "raw", "value", "raw_value", "rawvalue", "data", "payload", "body",
        }
        sanitized_mapping = {
            key: (
                REDACTED
...
def main() -> int:
    ...
    head_container = f"{args.cluster_id}_node_0"
    readiness_receipt = remote(
        HOSTS[0], f"docker exec {shlex.quote(head_container)} cat /tmp/glm53-postready.ok"
    )
    try:
        readiness_epoch = int(
            datetime.fromisoformat(readiness_receipt["stdout"].strip()).timestamp()
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise RuntimeError("cannot parse post-ready receipt timestamp") from exc
    if (
        readiness_receipt["returncode"] != 0
        or readiness_receipt["stderr"] != ""
        or not args.launch_epoch <= readiness_epoch <= acceptance["started_at"]
        or not acceptance["started_at"] <= acceptance["completed_at"]
    ):
        raise RuntimeError("invalid readiness-to-acceptance interval")
    acceptance_upper = acceptance["completed_at"] + 1.0
    ...
    for rank, host in enumerate(HOSTS):
        ...
        host_record = {
            ...
            "kernel_readiness_to_acceptance": remote(
                host,
                f"journalctl -k --since '@{readiness_epoch}' "
                f"--until '@{acceptance_upper:.6f}' --no-pager",
            ),
        ...
        for kernel_name in (
            ...
            kernel_receipt = host_record[kernel_name]
            if (
                kernel_receipt.get("returncode") != 0
                or kernel_receipt.get("stderr") != ""
            ):
                raise RuntimeError(f"{host} {kernel_name} capture was incomplete")
        if host == HOSTS[0]:
            host_record["postready_rc"] = remote(host, f"docker exec {qcontainer} cat /tmp/glm53-postready.rc")
            host_record["postready_ok"] = readiness_receipt
